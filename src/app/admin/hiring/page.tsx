...
export default function AdminHiring() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (reqRes.error) {
          console.error("Error fetching hiring requests:", reqRes.error)
          throw reqRes.error
        }
        if (depRes.error) {
          console.error("Error fetching departments:", depRes.error)
          throw depRes.error
        }
  ...
  const handleAddRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (Number.isNaN(positionsRequested) || positionsRequested <= 0) {
        alert("Positions requested must be a positive number")
        return
      }

      if (positionsApproved < 0 || positionsApproved > positionsRequested) {
        alert("Positions approved must be between 0 and positions requested")
        return
      }
      ...
      console.error("Error adding hiring request:", err)
      alert(`Failed to add hiring request: ${err.message || "Unknown error"}`)
    }
  }
  ...
  const handleUpdateRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`Failed to update hiring request: ${err.message || "Unknown error"}`)
    }
  }
  ...
  const handleDeleteRequest = async (id: string) => {
    ...
    try {
      ...
      alert(`Failed to delete hiring request: ${err.message || "Unknown error"}`)
    }
  }
  ...
  const handleApproveRequest = async (req: HiringRequest) => {
    try {
      ...
      alert(`Failed to start approval process: ${err.message || "Unknown error"}`);
    }
  };
  ...
  const handleUserCreated = async (userId: string) => {
    ...
    try {
      ...
      if (error) {
        console.error("Error approving hiring request after user creation:", error);
      ...
      console.error("Error in user created callback:", err);
      alert(`Failed to complete approval: ${err.message || "Unknown error"}`);
    }
  };

  const handleCancelUserCreation = () => {
    // When user cancels the modal, do nothing - request stays pending
  ...
  const handleRejectRequest = async (req: HiringRequest, reason: string) => {
    ...
    try {
      ...
      console.error("Error rejecting hiring request:", err)
      alert(`Failed to reject: ${err.message || "Unknown error"}`)
    }
  }

  // Function to change status of approved/rejected requests
  const handleChangeStatus = async (req: HiringRequest, newStatus: HiringStatus, reason?: string) => {
    try {
      ...
      if (newStatus === "rejected") {
        updateData.rejection_reason = reason || "Status changed by admin";
        updateData.approval_notes = null;
      } else if (newStatus === "approved") {
        updateData.approval_notes = reason || "Approved by admin";
  ...
  const handleExportRequests = async () => {
    try {
      ...
      a.href = url
      a.download = `hiring-requests-${new Date().toISOString().split("T")[0]}.csv`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      window.URL.revokeObjectURL(url)
    } catch (err: any) {
      console.error("Error exporting hiring requests:", err)
      alert(`Failed to export hiring requests: ${err.message || "Unknown error"}`)
    }
  }
  ...
  const filteredRequests = requests.filter((r) => {
    ...
    const haystack = `${r.title} ${r.role_type} ${r.employment_type} ${dep?.name || ""}`.toLowerCase()
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          <p className="text-gray-600">Loading hiring management...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Hiring Management</h1>
              <p className="text-gray-600">
                Manage and approve hiring requests for staff, faculty, HODs, HR and finance
              </p>
            </div>
          ...
          <Card className="lg:col-span-2 bg-white border-gray-200">
            <CardHeader>
              ...
              <CardDescription className="text-gray-600">
                Review, approve or reject hiring requests
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {filteredRequests.length === 0 ? (
                <p className="text-gray-600 text-sm">No hiring requests found for current filters.</p>
              ) : (
                          ...
                          <div className="text-sm text-gray-600 mt-1 space-y-1">
                            ...
                            {(req.expected_joining_from || req.expected_joining_to) && (
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <span>
                                  {req.expected_joining_from &&
                                    new Date(req.expected_joining_from).toLocaleDateString()}
                                  {req.expected_joining_to && (
                                    <>
                                      {" - "}
                                      {new Date(req.expected_joining_to).toLocaleDateString()}
                                    </>
                                  )}
                                </span>
                              </div>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <p className="text-xs text-gray-500 mt-1">All hiring requests in the system</p>
                </div>
            ...
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-xl">
              <div className="p-6 border-b border-gray-200">
                ...
                <p className="text-gray-600 text-sm mt-1">
                  Create a new hiring request for staff, faculty, HOD, HR or finance
                </p>
              </div>
              <div className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, role_type: e.target.value as RoleType })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      >
                        <option value="faculty">Faculty</option>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, priority: e.target.value as PriorityType })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, positions_requested: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, positions_approved: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-900 mb-1">
                        Expected Joining From
                      </label>
                      <input
                        type="date"
                        value={formData.expected_joining_from}
                        onChange={(e) =>
                          setFormData({ ...formData, expected_joining_from: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, expected_joining_to: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-xl">
              <div className="p-6 border-b border-gray-200">
                ...
                <p className="text-gray-600 text-sm mt-1">
                  Update or approve this hiring request
                </p>
              </div>
              <div className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, status: e.target.value as HiringStatus })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      >
                        <option value="pending">Pending</option>
